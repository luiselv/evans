...
/// <summary>
/// Shared integration test fixture for Comprobante infrastructure tests.
/// Primary: MsSqlContainer (Testcontainers) — requires Docker.
/// Fallback: LocalDB (MSSQLLocalDB) — used when Docker is not available.
/// Reuses the same schema-yearly.sql that GuiaRepositoryFixture uses.
/// </summary>
public class ComprobanteRepositoryFixture : IAsyncLifetime
{
    private const string LocalDbServer = @"(LocalDb)\MSSQLLocalDB";
    private const string LocalDbMasterName = "EVANS_ComprobanteTest";
    private const string LocalDbYearlyName = "ComprobanteTest_Year";
    ...
    public const string MasterDbName = "EVANS_ComprobanteTest";
    public const string YearlyDbName = "ComprobanteTest_Year";
    ...
    {
        if (await TableExistsAsync(YearlyConnectionString, "Comprobante")) return;
    ...
    private async Task SeedAsync()
    {
        ...
        await masterConn.ExecuteAsync(@"
            ...
            IF NOT EXISTS (SELECT 1 FROM TIPOCOMPROBANTE WHERE TICO_CODIGO = 2)
                INSERT INTO TIPOCOMPROBANTE (TICO_CODIGO, TICO_DESCRIPCION, ESTA_CODIGO) VALUES (2, 'Boleta', 1);
            ...
            IF NOT EXISTS (SELECT 1 FROM CLIENTE WHERE CLIE_CODIGO = 1)
                ...
                VALUES (1, 'Cliente Test SA', 1, '20111111111');
            IF NOT EXISTS (SELECT 1 FROM CLIENTE WHERE CLIE_CODIGO = 2)
                INSERT INTO CLIENTE (CLIE_CODIGO, CLIE_NOMBRE, IDEN_CODIGO, CLIE_NROIDENTIFICACION)
                VALUES (2, 'Cliente DNI Test', 2, '12345678');
