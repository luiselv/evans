...
public partial class frmMantCarreta : Form
{
    private readonly IMediator? _mediator;
    private IReadOnlyList<CarretaDto> _carretas = [];
    ...
    public frmMantCarreta() => InitializeComponent();

    public frmMantCarreta(IMediator mediator)
    {
        _mediator = mediator;
        InitializeComponent();
        WireEvents();
    ...
    internal async Task OpenFirstForTestAsync()
    {
        if (lvListado.Items.Count > 0)
            await OpenCarretaAsync(int.Parse(lvListado.Items[0].SubItems[0].Text, CultureInfo.InvariantCulture));
    }

    internal async Task LoadCarretasAsync()
    {
        await EnsureCombosAsync();
        _carretas = await Mediator.Send(new ListCarretasMaintenanceQuery());
        BindCarretas(_carretas);
    }
    ...
    internal async Task<bool> BuscarAsync(bool showMessages = false)
    {
        ...
        if (_carretas.Count == 0)
            _carretas = await Mediator.Send(new ListCarretasMaintenanceQuery());

        var search = txtBuscar.Text.Trim();
        BindCarretas(_carretas.Where(c => c.Placa.StartsWith(search, StringComparison.CurrentCultureIgnoreCase)));
    ...
    internal async Task<bool> SaveAsync(bool showMessages = false)
    {
        ...
        var certificado = string.IsNullOrWhiteSpace(txtCertificado.Text) ? null : txtCertificado.Text.Trim().ToUpperInvariant();

        if (_isCreateMode)
        {
            var result = await Mediator.Send(new CreateCarretaCommand(placa, marca, certificado, empresaCodigo.Value, estadoCodigo.Value));
            if (!result.IsSuccess) return ShowFailure(result.Error, showMessages);
        }
        else
        {
            var result = await Mediator.Send(new UpdateCarretaCommand(
                int.Parse(txtCodigo.Text, CultureInfo.InvariantCulture), placa, marca, certificado, empresaCodigo.Value, estadoCodigo.Value));
            if (!result.IsSuccess) return ShowFailure(result.Error, showMessages);
            _isCreateMode = true;
        }
    ...
    private async Task lvListado_DoubleClick()
    {
        if (lvListado.SelectedItems.Count > 0)
            await OpenCarretaAsync(int.Parse(lvListado.SelectedItems[0].SubItems[0].Text, CultureInfo.InvariantCulture));
    }

    private async Task OpenCarretaAsync(int codigo)
    {
        await EnsureCombosAsync();
        var carreta = await Mediator.Send(new GetCarretaByIdQuery(codigo));
        if (carreta is null) return;

        tabPageDetalles.Enabled = true;
        tabPageListado.Enabled = false;
        tabControl1.SelectTab(tabPageDetalles);
        txtCodigo.Text = carreta.Codigo.ToString(CultureInfo.InvariantCulture);
        txtMarca.Text = carreta.Marca;
        txtPlaca.Text = carreta.Placa;
        txtCertificado.Text = carreta.Certificado;
        SelectEmpresa(carreta.EmpresaCodigo);
        SelectEstado(carreta.EstadoCodigo);
    ...
    private void BindCarretas(IEnumerable<CarretaDto> carretas)
    {
        lvListado.Items.Clear();
        foreach (var carreta in carretas)
        {
            var item = lvListado.Items.Add(carreta.Codigo.ToString(CultureInfo.InvariantCulture));
            item.SubItems.Add(carreta.Marca ?? string.Empty);
            item.SubItems.Add(carreta.Placa);
            item.SubItems.Add(GetEmpresaName(carreta.EmpresaCodigo));
        }
    }
    ...
    {
        var message = error ?? "No se pudo guardar la carreta.";
        SetStatus(message);
